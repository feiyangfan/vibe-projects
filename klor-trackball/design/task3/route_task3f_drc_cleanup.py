...
def parse_pads(text: str) -> list[Pad]:
    ...
    for fp in blocks(text, "footprint"):
        ...
        for p in blocks(fp, "pad"):
            ...
            # KiCad custom pads can have copper primitives much larger than
            # their tiny anchor size (JP16 is the relevant stock example).
            # Use a conservative symmetric primitive envelope so routing
            # cannot clip a custom pad that the old 3E router under-modeled.
            if "(primitives" in p:
                primitive_xy = [
                    (float(x), float(y))
                    for x, y in re.findall(r"\(xy\s+([-\d.]+)\s+([-\d.]+)\)", p)
                ]
                if primitive_xy:
                    sx = max(sx, 2 * max(abs(x) for x, _ in primitive_xy))
                    sy = max(sy, 2 * max(abs(y) for _, y in primitive_xy))
            result.append(Pad(
                (gx, gy),
                (sx, sy),
                angle,
...
def parse_keepout_polygons(text: str) -> list[list[tuple[float,float]]]:
    result = []
    for z in blocks(text, "zone"):
        if "(keepout" not in z or "(tracks not_allowed)" not in z:
            continue
        pts = [(float(x), float(y)) for x,y in re.findall(r"\(xy\s+([-\d.]+)\s+([-\d.]+)\)", z)]
        if len(pts) >= 3:
            result.append(pts)
    return result


def parse_copper_graphic_polygons(text: str) -> list[tuple[str,list[tuple[float,float]]]]:
    result = []
    # Top-level/imported graphic copper polygons are already expressed in
    # board coordinates. Custom-pad primitive gr_poly blocks have no layer
    # atom of their own and are therefore excluded here.
    for token in ("gr_poly",):
        for b in blocks(text, token):
            lm = re.search(r'\(layer\s+"([^"]+)"\)', b)
            if not lm or lm.group(1) not in LAYERS:
                continue
...
def segment_polygon_distance(a, b, poly):
    if inside_polygon(a, poly) or inside_polygon(b, poly):
        return 0.0
    return min(
        segment_segment_distance(a, b, poly[i-1], poly[i])
...
def point_polygon_distance(p, poly):
    if inside_polygon(p, poly):
        return 0.0
    return min(
        point_segment_distance(p, poly[i-1], poly[i])
        for i in range(len(poly))
    )
...
def main() -> int:
    if git_blob(PCB) != BASELINE_BLOB:
        raise SystemExit("Task 3F rerouter requires the exact Task 3D PCB baseline")

    text = PCB.read_text(encoding="utf-8")
    if "(filled_polygon" in text:
        raise SystemExit("zone-fill cache must be stripped before Task 3F rerouting")
    ...
    route_json = {
        r.name: {
            "net": r.net,
            "segments": [
                {"a": list(seg.a), "b": list(seg.b), "width": seg.width, "layer": seg.layer}
                for seg in r.segments
            ],
            "vias": [
                {"p": list(v.p), "size": v.size, "drill": v.drill}
                for v in r.vias
            ],
            "length_mm": route_length(r),
        }
        for r in routes
    }
    print(f"Task 3F candidate inserted {sum(len(r.segments) for r in routes)} segments and {sum(len(r.vias) for r in routes)} vias")
    print(f"Candidate PCB blob: {actual}")
    print("TASK3F_ROUTE_JSON_BEGIN")
    print(json.dumps(route_json, sort_keys=True))
    print("TASK3F_ROUTE_JSON_END")
    return 0
